...
class TestAIInteractionEndpoints:
    async def test_create_ai_interaction(self, client, test_user, test_project, auth_headers):
    ...
    async def test_read_ai_interaction(self, client, test_user, test_ai_interaction, auth_headers):
        with patch.object(AuthService, 'get_current_user', return_value=test_user):
            response = client.get(
